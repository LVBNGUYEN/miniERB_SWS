  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
      <div 
        ...
        onClick={onClose}
      />
      <div className="relative w-full max-w-2xl bg-bg-card border border-white/10 rounded-3xl shadow-2xl overflow-hidden animate-in zoom-in-95 fade-in duration-300">
        {/* Header */}
        <div className="px-8 py-6 border-b border-border-primary flex items-center justify-between bg-bg-card">
          <h3 className="text-xl font-bold text-text-primary tracking-tight">{title}</h3>
          <button 
            onClick={onClose}
            className="p-2 rounded-xl bg-bg-surface text-text-secondary hover:text-text-primary hover:bg-slate-700/10 dark:hover:bg-slate-800 transition-all font-bold"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        {/* Content */}
        <div className="px-8 py-8 bg-bg-card">
          {children}
        </div>
